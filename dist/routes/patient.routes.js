...
patientRoutes.delete('/Delete/:id', userValidation_middleware_1.default.validation, (req, res) => {
    patientCtr.delete(req.params.id)
        .then(patient => {
        return res.status(200).send(new jsonResp_1.default(true, 'Paciente eliminado crrectamente', patient));
    })
        .catch((error) => {
        return res.status(500).send(new jsonResp_1.default(false, error.name || 'Error!', null, error));
    });
});
patientRoutes.get('/getPatientsNotEvaluated/:id', userValidation_middleware_1.default.validation, (req, res) => {
