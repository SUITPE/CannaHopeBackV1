...
const bankController_1 = __importDefault(require("../controllers/patientManagement/bankController"));
const medicationController_1 = __importDefault(require("../controllers/patientManagement/medicationController"));
...
const medicationCtr = new medicationController_1.default();
const bankCtr = new bankController_1.default();
...
patientManagementRoutes.post('/medication/save', userValidation_middleware_1.default.validation, (req, res) => {
    medicationCtr.save(req.body)
        .then(medication => {
        return res.status(200).send(new jsonResp_1.default(true, 'Medications guardado corectamente', medication));
    })
        .catch(error => {
        return res.status(500).send(new jsonResp_1.default(false, 'Error al guardar medications', null, error));
    });
});
patientManagementRoutes.get('/medication/findAll', userValidation_middleware_1.default.validation, (req, res) => {
    medicationCtr.findAll()
        .then(medicationList => {
        return res.status(200).send(new jsonResp_1.default(true, 'Lista de medications cargada correctamente', medicationList));
    })
        .catch(error => {
        return res.status(500).send(new jsonResp_1.default(false, 'Error al cargar lista de medications', null, error));
    });
});
patientManagementRoutes.delete('/medication/delete/:id', userValidation_middleware_1.default.validation, medicationCtr.delete);
patientManagementRoutes.post('/bank/save', userValidation_middleware_1.default.validation, (req, res) => {
    bankCtr.save(req.body)
        .then(bank => {
        return res.status(200).send(new jsonResp_1.default(true, 'Banca guardado corectamente', bank));
    })
        .catch(error => {
        return res.status(500).send(new jsonResp_1.default(false, 'Error al guardar banca', null, error));
    });
});
patientManagementRoutes.get('/bank/findAll', userValidation_middleware_1.default.validation, (req, res) => {
    bankCtr.findAll()
        .then(bankList => {
        return res.status(200).send(new jsonResp_1.default(true, 'Lista de bancos cargada correctamente', bankList));
    })
        .catch(error => {
        return res.status(500).send(new jsonResp_1.default(false, 'Error al cargar lista de bancos', null, error));
    });
});
patientManagementRoutes.delete('/bank/delete/:id', userValidation_middleware_1.default.validation, bankCtr.delete);
